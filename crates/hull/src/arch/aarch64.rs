...
pub unsafe fn init_boot_cpu() {
    ...
    unsafe {
        ...
        asm!(include_str!("../asm_fragments/arch-aarch64-inline-06.s"),
            vbar = in(reg) vbar,
...
fn halt_forever() -> ! {
    loop {
        ...
        unsafe {
            asm!(
                include_str!("../asm_fragments/arch-aarch64-inline-05.s"),
                options(nomem, nostack, preserves_flags)
            );
        }
    }
...
global_asm!(include_str!("../asm_fragments/arch-aarch64-global-02.s"));
...
pub unsafe fn sync_instruction_cache(addr: u64, len: usize) {
    ...
    unsafe {
        let mut p = base;
        while p < end {
            asm!(include_str!("../asm_fragments/arch-aarch64-inline-04.s"), p = in(reg) p, options(nostack, preserves_flags));
            p += 16;
        }
        asm!(
            include_str!("../asm_fragments/arch-aarch64-inline-03.s"),
            options(nostack, preserves_flags)
        );
        let mut p = base;
        while p < end {
            asm!(include_str!("../asm_fragments/arch-aarch64-inline-02.s"), p = in(reg) p, options(nostack, preserves_flags));
            p += 16;
        }
        asm!(
            include_str!("../asm_fragments/arch-aarch64-inline-01.s"),
            options(nostack, preserves_flags)
        );
    }
}
...
global_asm!(include_str!("../asm_fragments/arch-aarch64-global-01.s"));
